...
| RuleSpec NZ [#80](https://github.com/TheAxiomFoundation/rulespec-nz/pull/80) | Closes [#79](https://github.com/TheAxiomFoundation/rulespec-nz/issues/79) | Both In Progress | `CHANGES_REQUESTED`; hosted `validate / validate` and `validate / validate (nz)` fail while shard, roadmap, and quality checks pass; the Axiom maintainer says the old signing-key path is superseded and will carry the verified schema correction into the canonical corpus/Ed25519 migration | Axiom carries the source fix into the canonical migration and reviews/merges/declines; contributor leaves #80 open as evidence |
...
- No maintainer approval, canonical migration, or merge is inferred from contributor evidence.
